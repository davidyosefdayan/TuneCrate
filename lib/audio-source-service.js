...
class AudioSourceService {
    ...
    getDownloadUrl(videoId) {
        ...
        if (session) {
            session.lastAccessedAt = Date.now();
            console.log(`[DOWNLOAD] Reusing preview session URL for ${videoId}`);
            return session.remoteUrl;
        }

        const cached = this.getCachedAudioUrl(videoId);
        if (cached) {
            console.log(`[DOWNLOAD] Reusing cached media URL for ${videoId}`);
            return cached.url;
        }
    ...
    resolveDownloadUrl(videoId) {
        return this.getDownloadUrl(videoId);
    }
