...
function validateCandidate(candidate) {
    try {
        ...
        // If it's an absolute path to an existing executable, trust it
        // (execFileSync may fail in restricted environments like DaVinci Resolve)
...
function resolveYtdlpPath(settings, baseDir) {
    ...
    for (const candidate of candidates) {
        if (candidate !== 'yt-dlp' && !fs.existsSync(candidate)) {
            console.log(`[yt-dlp] Skipping (not found): ${candidate}`);
            continue;
        }
